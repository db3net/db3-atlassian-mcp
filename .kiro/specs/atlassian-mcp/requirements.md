...
This feature extends the existing Jira MCP server with Confluence read/write support and Jira write capabilities. Currently, the server only supports reading Jira tickets. The goal is to provide a unified interface for interacting with both Jira and Confluence — reading, creating, and updating content across both platforms — without leaving the development workflow.
...
- **MCP_Server**: The existing FastMCP server process (`server.py`) that exposes tools to MCP clients
- **Confluence_API**: The Atlassian Confluence REST API v2 (and v1 where needed) at `{base_url}/wiki/api/v2` and `{base_url}/wiki/rest/api`
- **Page**: A Confluence page identified by its numeric page ID, containing a title and body in Atlassian Document Format (ADF) or storage format
...
- **ADF**: Atlassian Document Format, a JSON-based document representation used by Confluence API v2
- **Jira_API**: The Atlassian Jira REST API v3 at `{base_url}/rest/api/3`
- **Jira_Ticket**: A Jira issue identified by its issue key (e.g. `INFOSEC-2239`), containing a summary, description, status, assignee, priority, and issue type
...
1. THE MCP_Server SHALL authenticate Confluence_API requests using the same JIRA_USER and JIRA_API_KEY environment variables used for Jira API requests
2. THE MCP_Server SHALL construct Confluence_API URLs using the same JIRA_BASE_URL environment variable with the `/wiki` path prefix
...
1. THE MCP_Server SHALL provide a Confluence-specific API helper function that prepends `/wiki/api/v2` to the given path
2. THE MCP_Server SHALL reuse the existing `_auth_header` function for authentication in the Confluence API helper
3. WHEN the Confluence_API returns an HTTP error, THE MCP_Server SHALL return a JSON object containing the HTTP status code and error body, consistent with the existing Jira error handling pattern
...
3. WHEN an optional description string is provided, THE MCP_Server SHALL include the description in the created Jira_Ticket as ADF content
4. WHEN optional fields (assignee account ID, priority name) are provided, THE MCP_Server SHALL set those fields on the created Jira_Ticket
5. IF the project key is invalid or the user lacks create permission, THEN THE MCP_Server SHALL return a JSON object containing the HTTP error code and error detail from the Jira_API
...
1. WHEN an issue key and one or more editable fields (summary, description, assignee account ID, priority name) are provided, THE MCP_Server SHALL update the specified fields on the Jira_Ticket using the Jira_API
...
6. IF the issue key does not exist or the user lacks edit permission, THEN THE MCP_Server SHALL return a JSON object containing the HTTP error code and error detail from the Jira_API
...
2. THE install script SHALL be runnable directly from GitHub via `curl -sSL https://raw.githubusercontent.com/{org}/{repo}/main/install.sh | bash` as a one-liner
3. WHEN the install script is run, it SHALL prompt the user for their Atlassian instance URL, email address, and API token
4. WHEN credentials are provided, the install script SHALL store them in a `.env` file inside the hidden venv directory (`~/.atlassian-mcp/.env`)
5. THE install script SHALL clone or download the server code into `~/.atlassian-mcp/`
6. THE install script SHALL create a Python virtual environment at `~/.atlassian-mcp` and install all required dependencies into it
7. WHEN the install script completes, it SHALL create or update the user's Kiro MCP config file (`~/.kiro/settings/mcp.json`) with the correct server entry
8. IF the virtual environment already exists, the install script SHALL reuse it and update dependencies rather than recreating it
9. THE install script SHALL validate that the provided credentials can authenticate against the Atlassian API and report success or failure to the user
