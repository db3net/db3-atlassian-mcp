...
- [x] 1. Add Confluence helper functions
  - [x] 1.1 Implement `_strip_html` to convert storage format XHTML to plain text
    - Strip HTML tags, decode entities, preserve line breaks from `<p>`, `<br/>`, `<li>`
    - Use `html.parser` or regex (no new dependencies)
    - _Requirements: 1.3_

  - [x] 1.2 Implement `_text_to_storage` to convert plain text to Confluence storage format
    - Split on newlines, wrap each non-empty line in `<p>...</p>`
    - _Requirements: 4.1, 5.1_

  - [x] 1.3 Implement `_parse_confluence_url` to extract page ID from URL or raw ID
    - Accept raw numeric IDs and full Confluence URLs (`/wiki/spaces/{KEY}/pages/{id}/...`)
    - Raise `ValueError` for unrecognized input
    - _Requirements: 1.2, 1.4_

  - [x] 1.4 Implement `_confluence_api` HTTP helper
...
- [x] 2. Implement Confluence read tools
  - [x] 2.1 Implement `get_confluence_page` tool
  ...
  - [x] 2.2 Implement `search_confluence` tool
  ...
  - [x] 2.3 Implement `get_space_pages` tool
...
- [x] 3. Checkpoint - Verify Confluence read tools
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Implement Confluence write tools
  - [x] 4.1 Implement `create_confluence_page` tool
  ...
  - [x] 4.2 Implement `update_confluence_page` tool
...
- [x] 5. Implement Jira write tools
  - [x] 5.1 Implement `_text_to_adf` helper to convert plain text to ADF JSON
    - Split on newlines, create paragraph nodes, return ADF doc structure
    - _Requirements: 8.3_

  - [x] 5.2 Implement `create_ticket` tool
  ...
  - [x] 5.3 Implement `update_ticket` tool
